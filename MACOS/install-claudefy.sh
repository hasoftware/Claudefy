...
have jq     || { stop_spin "jq is required (brew install jq)" 0; exit 1; }
...
if [ "$SKIP_FONT" = "1" ]; then
  ...
  if brew list --cask font-jetbrains-mono-nerd-font >/dev/null 2>&1; then
    ...
    stop_spin "Nerd Font — installed via Homebrew"
  else
    stop_spin "Nerd Font — brew not found, install manually" 0
  fi
fi
...
NEW=$(echo "$EXIST" | jq \
  ...
  .statusLine = ({ type: "command", command: ("bash \"" + $sl + "\"") }
                 + (if (.statusLine.refreshInterval // null) == null then {}
                    else { refreshInterval: .statusLine.refreshInterval } end))
  | .hooks = (.hooks // {})
