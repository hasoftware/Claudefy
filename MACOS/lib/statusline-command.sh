#!/usr/bin/env bash
# Claudefy statusLine - macOS/bash port
...
# is the whole point. No /proc on macOS, so this shells out to
# top/vm_stat/sysctl instead — still bounded by the TTL.
# ---------------------------------------------------------------------------
...
if [ "$COMPACT" != "1" ] && [ -n "$branch" ] && command -v gh >/dev/null 2>&1; then
  ...
  if [ -f "$cache_file" ]; then
    ...
    if [ -n "$ts" ]; then
      cache_age=$(( $(date -u +%s) - $(date -u -j -f '%Y-%m-%dT%H:%M:%SZ' "$ts" +%s 2>/dev/null) ))
...
if [ "$COMPACT" != "1" ]; then
  ...
  if echo "$batt_out" | grep -q 'InternalBattery'; then
    batt_pct=$(echo "$batt_out" | grep -oE '[0-9]+%' | head -1 | tr -d '%')
    batt_state=$(echo "$batt_out" | grep -oE 'discharging|charging|charged' | head -1)
...
# Clock (Hanoi)
now_hanoi=$(date -u -v+7H +'%H:%M' 2>/dev/null)
...
if [ -f "$uc_file" ]; then
  ...
  if [ -n "$uc_ts" ]; then
    uc_age=$(( $(date -u +%s) - $(date -u -j -f '%Y-%m-%dT%H:%M:%SZ' "$uc_ts" +%s 2>/dev/null) ))
...
if [ "$COMPACT" != "1" ] && [ -n "$cwd" ] && [ -d "$cwd" ]; then
  ...
  need_ec=true
  if [ -f "$env_cf" ]; then
    ec_mtime=$(stat -f %m "$env_cf" 2>/dev/null)
    if [ -n "$ec_mtime" ]; then
...
if [ "$COMPACT" != "1" ] && [ -d "$HOME/.claude/projects" ]; then
  ...
  if [ -z "$st_age" ] || [ "$st_age" -ge 3600 ] 2>/dev/null; then
    st_dates=$(find "$HOME/.claude/projects" -name '*.jsonl' -mtime -40 \
      -exec stat -f '%Sm' -t '%Y-%m-%d' {} + 2>/dev/null | sort -u)
    streak=0; sd_i=0
    while [ "$sd_i" -lt 40 ]; do
      d=$(date -v-${sd_i}d +%Y-%m-%d 2>/dev/null)
      [ -z "$d" ] && break
